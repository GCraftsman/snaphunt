...
function createSubmissionPopupHtml(sub: CompletedSubmission, teamColor: string, teamName: string): string {
  ...
  const scoreLabel = (sub.bonusPoints || 0) > 0 ? `${sub.points || 0}+${sub.bonusPoints}b` : `${sub.points || 0}`;
  ...
  return `<div style="text-align:center;min-width:140px;">
    ...
    ${!sub.photoData ? `<div style="font-size:12px;color:gold;font-weight:bold;">+${scoreLabel} pts</div>` : ""}
  </div>`;
}
